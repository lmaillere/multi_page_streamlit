...
def app1(prev_vars):
    ...
    K = 10.0
    r = st.slider('Enter the Intrinsic growth rate (r):',min_value=0.2, max_value=4.0, step=0.1)
...
def app2(prev_vars):
    ###############################################################
    # plot the logistic equation in pyplot with interaction on param
    st.title("Plot of the logistic model 2")
...
app.add_app("logistic model 2", app2)
app.run()
